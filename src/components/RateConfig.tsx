...
import { DollarSign, ArrowRight, Settings2, Star } from 'lucide-react';
...
// Predefined standard categories (must match App.tsx)
const STANDARD_CATEGORIES = [
    'Mini', 'Infantil', 'Juvenil', 'Junior', 'Femenino', 'Senior',
    'U6-U8', 'U9-U11', 'U12-U14', 'U15-U16', 'U17-U19', 'U20+'
];
...
export const RateConfig: React.FC<RateConfigProps> = ({ categories, onConfirm, initialRates = {} }) => {
    ...
    // Separate categories into standard and additional
    const standardCategories = categories.filter(cat =>
        STANDARD_CATEGORIES.includes(cat)
    );
    ...
    const renderCategoryInput = (category: string, index: number, isStandard: boolean = false) => (
        <motion.div
            key={category}
            ...
            className="group"
        >
            <label className="block text-sm font-medium text-slate-700 mb-2 group-hover:text-primary-600 transition-colors flex items-center gap-2">
                {category}
                {isStandard && (
                    <Star className="w-3 h-3 text-amber-500 fill-amber-500" title="Standard Category" />
                )}
            </label>
        ...
        >
            <div className="p-8 border-b border-slate-100 bg-slate-50/50 flex items-center gap-4">
                ...
                <div>
                    ...
                    <p className="text-slate-500">Assign payment rates for each category. Standard categories are always available.</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-8">
                ...
                {standardCategories.length > 0 && (
                    <div>
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <h3 className="text-lg font-semibold text-slate-700">Standard Categories</h3>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            {standardCategories.map((category, index) =>
                                renderCategoryInput(category, index, true)
                            )}
                        </div>
                    </div>
                )}
                ...
                {additionalCategories.length > 0 && (
                    <div>
                        <h3 className="text-lg font-semibold text-slate-700 mb-4">Additional Categories</h3>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            {additionalCategories.map((category, index) =>
                                renderCategoryInput(category, index + standardCategories.length, false)
                            )}
                        </div>
                    </div>
                )}
